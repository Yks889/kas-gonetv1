...
<div class="container-fluid">
    ...
    <div class="d-flex justify-content-between align-items-center pb-3 mb-4 border-bottom border-secondary">
        <h1 class="h3 mb-0 text-white">
            <i class="bi bi-pencil-square me-2"></i> Edit Kas Keluar
        </h1>
        <a href="<?= site_url('admin/kas_keluar') ?>" class="btn btn-sm btn-outline-light stylish-btn">
    ...
    <div class="dashboard-card p-4">
        <form method="post" action="<?= site_url('admin/kas_keluar/update/' . $pengajuan['id']) ?>" class="needs-validation" novalidate>
            <?= csrf_field() ?>
            <div class="row g-3">
                <!-- Nominal Input -->
                <div class="col-md-6">
                    ...
                    <div class="input-group">
                        ...
                        <input type="text"
                            ...
                            id="nominal"
                            name="nominal"
                            placeholder="Masukkan nominal kas keluar"
                            value="<?= old('nominal', number_format($pengajuan['nominal'], 0, ',', '.')) ?>"
                            required>
                ...
                <div class="col-12">
                    ...
                    <textarea class="form-control modern-input <?= (isset($validation) && $validation->hasError('keterangan')) ? 'is-invalid' : '' ?>"
                        ...
                        name="keterangan"
                        rows="4"
                        placeholder="Masukkan keterangan kas keluar (contoh: Pembelian barang, Biaya operasional, dll.)"
                        required><?= old('keterangan', $pengajuan['keterangan']) ?></textarea>
                    <div class="invalid-feedback">
                        <?= isset($validation) ? $validation->getError('keterangan') : 'Keterangan harus diisi' ?>
                    </div>
                    <div class="form-text text-light">Jelaskan alasan dan tujuan pengeluaran kas</div>
                </div>

                <!-- Action Buttons -->
                <div class="col-12 mt-4">
                    <div class="d-flex gap-2 justify-content-end">
                        <a href="<?= site_url('admin/kas_keluar') ?>" class="btn btn-outline-light stylish-btn">
...
<script>
    ...
    // Inisialisasi nilai raw dari nilai awal yang sudah diformat
    document.addEventListener('DOMContentLoaded', function() {
        const initialValue = nominalInput.value.replace(/\D/g, '');
        nominalInput.dataset.raw = initialValue;
    });
</script>
